...
export class InsightsAnalystChatService {
	...
	private parseLlmResponse(
		...
		const workflowIds = new Set(workflowRows.flatMap(({ workflowId }) => (workflowId ? [workflowId] : [])));
	...
	private fallbackAnswer(
		...
		if (normalized.includes('fail')) {
			return {
				...
				citations: [this.createCitation(mostFailures, 'failed executions', mostFailures.failed, 'count')],
			};
		}
