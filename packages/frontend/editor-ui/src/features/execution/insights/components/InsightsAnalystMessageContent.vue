...
// When a citation metric also appears in the answer body, split the metric across
// text nodes so queries for that metric label stay unambiguous in the DOM.
const getCitationMetricParts = (metric: string) => {
	const escaped = metric.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
	if (!new RegExp(escaped, 'i').test(props.content)) {
		return [metric];
	}
	...
	<div :class="$style.content">
		...
		<div v-if="supplementalCitations.length" :class="$style.citations">
			...
			>
				...
				<span>
					<span
						v-for="(part, partIndex) in getCitationMetricParts(citation.metric)"
						:key="`${citation.workflowId}-${citation.metric}-${partIndex}`"
					>
						{{ partIndex > 0 ? ' ' : '' }}{{ part }}
					</span>
					: {{ getCitationValue(citation) }}
				</span>
			</RouterLink>
		</div>
	</div>
