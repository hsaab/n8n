	...
	<div :class="$style.insightsWrapper">
		<div :class="$style.insights">
			<ul data-test-id="insights-summary-tabs">
				<li
					v-for="{ id, rawValue, value, rawDeviation, deviation, deviationUnit, unit, to } in summaryWithRouteLocations"
					:key="id"
				...
				>
					...
					>
						...
						<component
							...
							:class="{ [$style.activeTab]: props.linkVariant === 'static' && id === activeInsightType }"
						...
						>
							...
							<span v-else>
								<em v-if="id === 'timeSaved'">{{ formatInsightsTimeSavedLabel(rawValue ?? value) }}</em>
								<em v-else
