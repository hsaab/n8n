...
// Shell tests mount without a vue-router plugin; InsightsSummary still reads useRoute().
provide(routeLocationKey, {
	name: VIEWS.INSIGHTS_ANALYST,
	params: {},
	query: {},
	meta: {},
	matched: [],
	fullPath: '/insights/analyst',
	path: '/insights/analyst',
	hash: '',
...
const toCalendarDateFromIso = (iso: string) => {
	const date = new Date(iso);
	return new CalendarDate(date.getUTCFullYear(), date.getUTCMonth() + 1, date.getUTCDate());
};

// Display-only: bind picker to seeded overview window; never refetch list endpoints on change.
watch(
	overview,
	(value) => {
		if (!value?.dateRange) return;
		range.value = {
			start: toCalendarDateFromIso(value.dateRange.startDate),
			end: toCalendarDateFromIso(value.dateRange.endDate),
		};
	},
	{ immediate: true },
);
